  ...
  if (!isOpen) {
    return (
      <button 
        ...
        className="bg-slate-950 dark:bg-white text-white dark:text-black px-6 py-3 text-[10px] font-black uppercase tracking-[0.2em] hover:bg-[#a855f7] hover:text-white transition-all cursor-pointer rounded-none border-none outline-none shadow-md"
      >
        New Deck
      </button>
    );
        ...
        <form onSubmit={handleSubmit} className="space-y-6 text-left">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <input name="company" required className="w-full bg-slate-50 dark:bg-white/[0.03] border border-transparent px-4 py-3 text-xs font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-[#a855f7] focus:bg-white dark:focus:bg-black/20 outline-none transition-all rounded-none" placeholder="e.g. Vercel" />
            </div>
            <div className="space-y-2">
              <label className="text-[9px] font-black uppercase tracking-widest opacity-40 text-slate-900 dark:text-white">URL Slug</label>
              <input name="slug" required className="w-full bg-slate-50 dark:bg-white/[0.03] border border-transparent px-4 py-3 text-xs font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-[#a855f7] focus:bg-white dark:focus:bg-black/20 outline-none transition-all rounded-none" placeholder="frontend-engineer" />
            </div>
          </div>

          <div className="space-y-2">
            <label className="text-[9px] font-black uppercase tracking-widest opacity-40 text-slate-900 dark:text-white">Resume PDF</label>
            <input type="file" name="resume_file" accept=".pdf" required className="w-full text-[10px] font-bold uppercase py-2 cursor-pointer text-slate-900 dark:text-white" />
          </div>
          ...
          <div className="space-y-2">
            ...
            <textarea name="cover_letter" rows={5} className="w-full bg-slate-50 dark:bg-white/[0.03] border border-transparent px-4 py-4 text-xs font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-[#a855f7] focus:bg-white dark:focus:bg-black/20 outline-none resize-none transition-all rounded-none" placeholder="Paste your tailored letter here..." />
          </div>

          <div className="flex items-center gap-4 bg-slate-50 dark:bg-white/[0.03] p-4 transition-colors">
            <input type="checkbox" name="is_public" id="is_public" className="accent-[#a855f7] w-4 h-4 cursor-pointer" />
          ...
          <button 
            disabled={loading}
            className="w-full bg-slate-950 dark:bg-white text-white dark:text-black py-5 text-[10px] font-black uppercase tracking-[0.2em] hover:bg-[#a855f7] hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer shadow-lg"
          >
